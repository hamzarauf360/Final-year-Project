...
var BTree1 = function() {
  ...
  this.inorder = function(Root) {
    if (Root) {
      ...
      inorder_traversal.push(Root.getData());
...
function question_3() {
  ...
  var txt2 = document.createTextNode("What is the inorder traversal of this bst?(Enter comma separated values)");
  ...
  document.getElementById("viewans3").addEventListener("click", function(e) {
    ...
    var nums = ""
    var i = 0;
    for (i = 0; i < inorder_traversal.length; i++) {


      nums += inorder_traversal[i];
      nums += ','
  ...
  document.getElementById("subin").addEventListener("click", function(e) {
    ...
    if (inorder_traversal.length != parsed.length) {
      var get = localStorage.getItem("bst_performance");
      get = get - 5;
      localStorage.setItem("bst_performance", get);
      swal({
      ...
      for (j = 0; j < inputt.length; j++) {
        if (inorder_traversal[j] != parsed[j]) {
